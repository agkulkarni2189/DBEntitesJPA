...
public class TransactionAPI {
	...
	public List<RFIDTransaction> getRFIDTransactions(String LocationID, String LaneTypeID, String LaneID, String FromCreationDate, String ToCreationDate) {
		...
		if (FromCreationDate != null && !FromCreationDate.isEmpty()) {
			if (WhereConditions.length() > 0)
				WhereConditions.append(" AND ltc.creation_time>='" + FromCreationDate+"'");
			else
				WhereConditions.append(" WHERE ltc.creation_time>='" + FromCreationDate+"'");
		}
		
		if (ToCreationDate != null && !ToCreationDate.isEmpty()) {
			if (WhereConditions.length() > 0)
				WhereConditions.append(" AND ltc.creation_time<'" + ToCreationDate+"'");
			else
				WhereConditions.append(" WHERE ltc.creation_time<'" + ToCreationDate+"'");
		}
		
		if (WhereConditions.length() > 0)
			TransactionsRetrievalQuery.append(WhereConditions.toString());

		javax.persistence.Query RFIDTransQuery = TransactionManager
				.createNativeQuery(TransactionsRetrievalQuery.toString(), RFIDTransaction.class);
